...
game_themes as (
    select
        gt.game_id,
        array_agg(distinct t.name order by t.name)::text [] as themes
    from {{ ref("bridge_game_theme") }} as gt
    inner join {{ ref("mart_themes") }} as t on gt.theme_id = t.id
    group by gt.game_id
),
...
game_genres as (
    select
        gg.game_id,
        array_agg(distinct gen.name order by gen.name)::text [] as genres
    from {{ ref("bridge_game_genre") }} as gg
    inner join {{ ref("mart_genres") }} as gen on gg.genre_id = gen.id
    group by gg.game_id
),
...
game_modes as (
    select
        gm.game_id,
        array_agg(distinct m.name order by m.name)::text [] as game_modes
    from {{ ref("bridge_game_game_mode") }} as gm
    inner join {{ ref("mart_game_modes") }} as m on gm.game_mode_id = m.id
    group by gm.game_id
),
...
game_platforms as (
    select
        gp.game_id,
        array_agg(distinct p.name order by p.name)::text [] as platforms
    from {{ ref("bridge_game_platform") }} as gp
    inner join {{ ref("mart_platforms") }} as p on gp.platform_id = p.id
    group by gp.game_id
),
...
game_engines as (
    ...
    from {{ ref("bridge_game_game_engine") }} as ge
    inner join {{ ref("mart_game_engines") }} as e on ge.game_engine_id = e.id
    group by ge.game_id
),
...
game_player_perspectives as (
    select
        gpp.game_id,
        array_agg(distinct pp.name order by pp.name)::text []
            as player_perspectives
    from {{ ref("bridge_game_player_perspective") }} as gpp
    inner join
        {{ ref("mart_player_perspectives") }} as pp
...
game_publishers as (
    ...
    inner join {{ ref("mart_companies") }} as c on ic.company = c.id
...
game_multiplayer_flags as (
    ...
    from {{ ref("bridge_game_multiplayer_mode") }} as gmm
    inner join
        {{ ref("mart_multiplayer_modes") }} as mm
        on gmm.multiplayer_mode_id = mm.id
...
game_multiplayer_modes as (
    ...
    from game_multiplayer_flags as gmf
),
...
select
    ...
    case
        when
            g.first_release_date_epoch is not null
            and g.first_release_date_epoch > 0
            then
                extract(
...
left join game_platforms as gpl on g.id = gpl.game_id
left join game_engines as ge on g.id = ge.game_id
left join game_player_perspectives as gpp on g.id = gpp.game_id
left join game_publishers as pub on g.id = pub.game_id
left join game_developers as dev on g.id = dev.game_id
left join game_multiplayer_modes as gmm on g.id = gmm.game_id
left join game_types as gtype on g.game_type = gtype.id
left join game_statuses as gs on g.game_status = gs.id
left join {{ ref("mart_covers") }} as img on g.cover = img.id
where g.name is not null and g.updated_at is not null
{% if is_incremental() %}
and g.updated_at::bigint > (select max(updated_at) from {{ this }})
{% endif %}
