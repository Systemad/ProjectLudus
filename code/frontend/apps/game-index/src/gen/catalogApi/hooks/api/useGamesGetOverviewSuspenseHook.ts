...
export const gamesGetOverviewSuspenseQueryKey = ({ gameId }: { gameId: GamesGetOverviewPathParams["gameId"] }) => ["v1", { url: '/api/games/:gameId', params: {gameId:gameId} }] as const
...
export async function gamesGetOverviewSuspenseHook({ gameId }: { gameId: GamesGetOverviewPathParams["gameId"] }, config: Partial<RequestConfig> & { client?: Client } = {}) {
  ...
  const res = await request<GamesGetOverviewQueryResponse, ResponseErrorConfig<GamesGetOverview404>, unknown>({ method : "GET", url : `/api/games/${gameId}`, ... requestConfig })
  return res.data
}
