...
{
    public static EntityMetadata<Franchise> Config { get; } =
        new(
            Endpoint: "franchises",
            Fields: ["*"],
            CopyCommand: "COPY game_engines (id, name, slug, url, updated_at, created_at, metadata, logo) FROM STDIN (FORMAT BINARY)",
