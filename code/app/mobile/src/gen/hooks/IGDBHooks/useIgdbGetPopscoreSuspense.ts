...
export function igdbGetPopscoreSuspenseQueryOptions({ query }: IgdbGetPopscoreOptions = {}, config: Partial<Omit<RequestConfig, 'path' | 'query' | 'body' | 'headers' | 'url'>> = {}) {
 ...
 */
export function useIgdbGetPopscoreSuspense<TData = IgdbGetPopscoreStatus200, TQueryKey extends QueryKey = IgdbGetPopscoreSuspenseQueryKey>({ query }: { query?: IgdbGetPopscoreOptions['query'] | (() => IgdbGetPopscoreOptions['query']) } = {}, options: {
  ...
  const queryResult = useSuspenseQuery({
   ...igdbGetPopscoreSuspenseQueryOptions(resolvedParams, config),
   ...resolvedOptions,
   queryKey,
