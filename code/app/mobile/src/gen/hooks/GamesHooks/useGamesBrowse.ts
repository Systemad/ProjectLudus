...
export function gamesBrowseQueryOptions({ query }: GamesBrowseOptions = {}, config: Partial<Omit<RequestConfig, 'path' | 'query' | 'body' | 'headers' | 'url'>> = {}) {
 ...
 */
export function useGamesBrowse<TData = GamesBrowseStatus200, TQueryData = GamesBrowseStatus200, TQueryKey extends QueryKey = GamesBrowseQueryKey>({ query }: { query?: GamesBrowseOptions['query'] | (() => GamesBrowseOptions['query']) } = {}, options: {
  ...
  const queryResult = useQuery({
   ...gamesBrowseQueryOptions(resolvedParams, config),
   ...resolvedOptions,
   queryKey,
