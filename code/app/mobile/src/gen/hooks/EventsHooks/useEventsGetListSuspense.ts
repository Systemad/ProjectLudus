...
export function eventsGetListSuspenseQueryOptions({ query }: EventsGetListOptions = {}, config: Partial<Omit<RequestConfig, 'path' | 'query' | 'body' | 'headers' | 'url'>> = {}) {
 ...
 * {@link /catalog/events}
 */
export function useEventsGetListSuspense<TData = EventsGetListStatus200, TQueryKey extends QueryKey = EventsGetListSuspenseQueryKey>({ query }: { query?: EventsGetListOptions['query'] | (() => EventsGetListOptions['query']) } = {}, options: {
  ...
  const queryResult = useSuspenseQuery({
   ...eventsGetListSuspenseQueryOptions(resolvedParams, config),
   ...resolvedOptions,
   queryKey,
