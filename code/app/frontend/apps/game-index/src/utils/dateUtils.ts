...
export const formatDateRange = (startUtc?: string | null, endUtc?: string | null) => {
    if (!startUtc) return "TBA";
    const fmt = new Intl.DateTimeFormat("en-US", { month: "short", day: "numeric" });
    const start = fmt.format(new Date(startUtc));
    const end = endUtc ? fmt.format(new Date(endUtc)) : null;
