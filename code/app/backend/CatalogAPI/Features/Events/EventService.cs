...
internal sealed class EventService(AppDbContext db) : IEventService
{
    public async Task<List<EventDto>> GetListAsync(
        int? year, int? month, string? status, int? limit, CancellationToken ct)
    {
        ...
        query = status?.ToLowerInvariant() switch
        {
            ...
            "finished" => query.Where(e => e.EndTimeUtc != null ? e.EndTimeUtc <= now : e.StartTimeUtc <= now),
        ...
        var rawEvents = await query
            ...
            {
                Event = e,
                GameIds = e.Games.Where(g => g.FirstReleaseDateUtc.HasValue).Select(g => g.Id).ToList(),
            })
            .ToListAsync(ct);

        var allGameIds = rawEvents.SelectMany(r => r.GameIds).Distinct().ToList();

        var gameLookup = allGameIds.Count > 0
            ? await db.Games.Where(g => allGameIds.Contains(g.Id))
                .SelectGameBrowseDto().ToListAsync(ct)
            : [];

        var gameMap = gameLookup.ToDictionary(g => g.Id);

        return rawEvents.Select(r => new EventDto
        {
            Id = r.Event.Id,
            Name = r.Event.Name,
            Slug = r.Event.Slug,
            ...
            Games = r.GameIds.Select(gameMap.GetValueOrDefault).Where(g => g != null).Select(g => g!).ToList(),
        }).ToList();
    }
    ...
    public async Task<EventDto?> GetByIdAsync(long id, CancellationToken ct)
    {
        ...
        var games = raw.GameIds.Count > 0
            ? await db.Games.Where(g => raw.GameIds.Contains(g.Id)).SelectGameBrowseDto().ToListAsync(ct)
            : [];
