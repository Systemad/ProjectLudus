...
public class GameBrowseDto
{
    ...
    [Required] public GameFeatures GameFeatures { get; set; } = new([], []);
...
public sealed record SteamData(long? SteamAppId, long? CurrentPlayers, long? Peak24h, string? HeaderUrl, string? CapsuleUrl);
...
public sealed record SteamReviewData(int? Score, string? Desc, int? TotalReviews, int? Positive, int? Negative);
